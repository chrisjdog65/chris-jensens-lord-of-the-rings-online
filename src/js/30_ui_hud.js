...
#hud.dead #notices, #hud.dead #interactPrompt, #hud.dead #castbar { visibility: hidden; }
...
`;
  ...
  UI.DeathScreen = {
    visible: false,
    show: function () { if (!ds.el) return; ds.el.hidden = false; UI.DeathScreen.visible = true; ds.t = DEATH_AUTO; ds.lastTxt = ''; UI.tooltip.hide(); if (_hudRoot) _hudRoot.classList.add('dead'); if (G.Input && typeof G.Input.exitLock === 'function') G.Input.exitLock(); },
    hide: function () { if (!ds.el) return; ds.el.hidden = true; UI.DeathScreen.visible = false; ds.t = -1; if (_hudRoot) _hudRoot.classList.remove('dead'); },
  };
