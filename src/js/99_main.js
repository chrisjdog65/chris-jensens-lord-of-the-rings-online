/* ==== 99_main.js — G.Game + window.__T: the integrator. Creates the renderer / scene / camera, runs the progressive
   ...
   Public API (G.Game):
     ...
     zoneMusic(force?)           G.Audio.music(zone theme | 'tavern' inside inns) + G.Audio.ambient(biome, G.Sky.phase);
                                 the playing loop also drives a positional 'fire_loop' at the nearest hearth of the
                                 building the player is inside (started/stopped here, synthesised by 01_audio);
...
(function () {
  ...
  // Hearth crackle: one positional 'fire_loop' voice (01_audio's fire bed) follows the nearest lit hearth of the
  // building the player is standing in — inn fireplaces, cottage hearths. Audio owns the sound; main only says
  // where the fire is, and stops the loop as soon as the player steps back outside.
  const _fireOpts = { pos: null, loop: true, vol: 0.6 };
  let fireT = 0, fireOn = false;
  function updateFireLoop(dt) {
    const A = G.Audio;
    if (!A || !A.ready || !has(A, 'sfx')) return;
    fireT -= dt; if (fireT > 0) return;
    fireT = 0.35;
    const B = G.Buildings, p = G.state.player;
    const bld = (B && p && p.pos) ? B.playerInside : null;
    let best = null, bd = 12 * 12;
    if (bld && bld.hearths) {
      for (let i = 0; i < bld.hearths.length; i++) {
        const h = bld.hearths[i];
        if (!h || !h.pos || h.kind === 'smoke') continue;
        const dx = h.pos.x - p.pos.x, dz = h.pos.z - p.pos.z, d2 = dx * dx + dz * dz;
        if (d2 < bd) { bd = d2; best = h; }
      }
    }
    if (best) { _fireOpts.pos = best.pos; try { A.sfx('fire_loop', _fireOpts); fireOn = true; } catch (e) { report(e, 'Game.fireLoop'); } }
    else if (fireOn) { fireOn = false; try { if (has(A, 'stopLoop')) A.stopLoop('fire_loop', 0.7); } catch (e) { /* ignore */ } }
  }
  ...
  function teardown() {
    ...
    try { if (G.Audio && has(G.Audio, 'stopLoop')) { G.Audio.stopLoop('fire_loop', 0.4); fireOn = false; } } catch (e) { /* ignore */ }
