...
- **In-game audio verification pass (fixes live in 01_audio unless noted):**
  ...
  * 99_main additionally drives a positional `fire_loop` (the previously unused `fire` ambient bed) at the nearest hearth of the building the player is inside, and `tools/verify.js` gained an **R45** scenario that checks all of this in the running game (zone theme, ambient bed, footsteps/UI/melee/damage cues, combat theme in and out, the burst guard, the victory one-shot handing back, the bus mix and the sliders, plus the fishing cast cue).
  * Harness gotcha that cost several runs: `G.Data.world.fishingSpots[i]` keeps its position in **`pos: {x, z}`**, not `x`/`z` (same shape as `towns`). Reading `s.x`/`s.z` teleports the player to `NaN`, `canFish()` then always answers "stand at the water's edge", and the fishing cues look silent when nothing is wrong. Under software GL also drive everything by `__T.waitFrames`, never wall-clock waits — 3 s of wall time can be a single frame — and `G.time.scale` is the lever for anything gated on game seconds (combat's 6 s drop-out costs 25 frames at scale 5 instead of ~120).
...
- Interiors: every enterable has a CEILING in the `int`/`upper` group that is never hidden (flat boards+joists, or the roof underside with rafters/purlins for halls and the inn's upper storey) plus a non-floor ceiling collider (`Physics.cameraClamp` keeps the chase camera under it, jumps bump it). Only the exterior `roof` group hides, while the player is inside (`bld.inside`), so a camera outside still looks into the room; `bld.camInside` reports whether the camera is in the interior volume but no longer drives visibility.
...
- **The bot's budget is RENDERED FRAMES, not game seconds** (software GL renders 0.2–1 fps here while the clock runs at 0.05 s × `G.time.scale` per frame). So: speed 10 teleports for *every* travel leg (speed ≥ 3 beyond 200 m), a tick that changed the world instantly chains into the next tick inside the same frame (≤ 6 deep), fights run `FIGHT_FRAMES` frames of abilities + auto-attack and are then finished by `G.Combat.kill` (with the rest of the pack the objective needs), and a fishing cast is credited after `CAST_FRAMES`. `stats` gained `chained`, `boats`, `packKills`.
- **Gotcha for every module (found via fishing):** `G.Player.getForward()` is built from the CAMERA yaw (`G.Player.cam.yaw`), not `player.yaw` — `26_fishing_boats`' `canFish`/`computeCastPoint` probe along it. Any scripted/AI actor that "turns to face" something must set `G.Player.cam.yaw` too, or facing-sensitive checks keep testing wherever the camera was left pointing. AutoQuest's `faceYaw()` sets body + camera + rig, and `stepFish` sweeps the facing against `canFish()` as an oracle.
- Full-run reference (fresh L1 man in Archet, B, speed 10, time ×5, render scale 0.25, shared 4-core box): **150/150 quests in 33.8 min / 1287 frames, level 80, all 18 slots in the Armour of the Lost Kingdom, 0 deaths, 0 `G.errors`**, 66 forced objectives, 1006 teleports, 23 boat hops.
...
## Pointer lock / mouse look (R44)
- Verified end-to-end: a real `page.mouse.click` on the canvas DOES get pointer lock in headless Chromium (`document.pointerLockElement === canvas`, `G.Input.mouse.locked` true); Escape and opening a panel release it, and a further canvas click re-acquires it.
